...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, meta=None):
...
class BaseBertProcessor(DataProcessor):
    ...
    def _create_examples(self, df, set_type):
        ...
        for data in df.values:
            ...
            label = data[0]
            meta = [0] * self.meta_shape
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, meta=meta))
        return examples
...
class BertGcnProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(self.train_df, self.gcn_train, "train")

    def get_test_examples(self, data_dir):
        """See base class."""
        # return self._create_examples(self.test_df, self.gcn_data, "test")
        return self._create_examples(self.test_df, self.gcn_test, "test")

    def get_labels(self):
        """See base class."""
        return [str(i) for i in range(len(self.lb.classes_))]

    def _create_examples(self, df, gcn_data_value, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        # gcn_examples = gcn_data_value[df.index]

        for data, gcn_example in zip(df.values, gcn_data_value):
            guid = "%s-%s" % (set_type, data[1])
            text_a = tokenization.convert_to_unicode(data[3])
            text_b = tokenization.convert_to_unicode(data[4])
            label = data[0]
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["meta"] = create_int_feature(feature.meta)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    name_to_features = {
        ...
        "meta": tf.FixedLenFeature([meta_length], tf.int64),
    }
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
    ...
    for feature in features:
        ...
        all_label_metas.append(feature.meta)
    ...
    def input_fn(params):
        ...
        d = tf.data.Dataset.from_tensor_slices({
            ...
            "meta":
                tf.constant(all_label_metas, shape=[num_examples], dtype=tf.int32)
        })
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        train_input_fn = file_based_input_fn_builder(
            ...
            is_training=True,
            drop_remainder=True,
            meta_length=processor.meta_shape)
